...
def knn(array):
    ...
    for i in range(number):
        ...
        for loop in range(k):  # k代表"K聚类"中的"K"是几
            if order5[loop, 0] == 10:
                ...
                classifier2 += 1
            elif order5[loop, 0] == 1000:
                classifier3 += 1
        if classifier1 >= classifier3 and classifier1 >= classifier2:
            result = 10
        if classifier2 >= classifier3 and classifier2 >= classifier1:
            result = 100
        if classifier3 >= classifier1 and classifier3 >= classifier2:
            result = 1000
        # print(result)
    ...
    if k == 1:
        print("iris数据集的", "最", "近邻准确率为：", accuracy)
    else:
        print("iris数据集的", k, "近邻准确率为：", accuracy)


iris = pandas.read_csv('iris.csv', header=None, sep=',')
